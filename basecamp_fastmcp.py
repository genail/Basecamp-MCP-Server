...
async def get_daily_check_ins(project_id: str, page: Optional[int] = None) -> Dict[str, Any]:
    ...
    try:
        if page is not None and not isinstance(page, int):
            page = 1
        questions = await _run_sync(lambda: client.get_daily_check_ins(project_id, page=page or 1))
        return {
            "status": "success",
            "check_ins": questions,
            "count": len(questions)
...
async def get_question_answers(project_id: str, question_id: str, page: Optional[int] = None) -> Dict[str, Any]:
    ...
    try:
        if page is not None and not isinstance(page, int):
            page = 1
        answers = await _run_sync(lambda: client.get_question_answers(project_id, question_id, page=page or 1))
        return {
            "status": "success",
            "answers": answers,
            "count": len(answers)
